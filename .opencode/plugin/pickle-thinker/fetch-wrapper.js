...
export function initializeFetchWrapper(config) {
...
function injectIntoOpenAIMessages(messages, prefix) {
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        if (msg.role === 'user' && Array.isArray(msg.content)) {
            const hasToolResult = msg.content.some((part) => part.type === 'tool_result');
            if (hasToolResult) {
                const failed = msg.content.some((part) => part.type === 'tool_result' && isToolOutputFailed(part.content));
                messages.splice(i + 1, 0, {
                    role: 'user',
                    content: buildThinkingPrompt(prefix, failed)
                });
...
function injectLitePrefix(messages, prefix) {
    for (let i = messages.length - 1; i >= 0; i--) {
        const msg = messages[i];
        if (msg.role === 'user') {
            if (typeof msg.content === 'string') {
...
function injectIntoAnthropicMessages(messages, prefix) {
    if (messages.length === 0)
        return false;
    let modified = false;
    for (const msg of messages) {
        if (msg.role === 'user' && Array.isArray(msg.content)) {
            const toolParts = msg.content.filter((part) => part.type === 'tool_result');
            if (toolParts.length > 0) {
                const failed = toolParts.some((part) => isToolOutputFailed(part.content));
                msg.content.push({
                    type: 'text',
...
function buildThinkingPrompt(prefix, failed) {
    if (failed) {
